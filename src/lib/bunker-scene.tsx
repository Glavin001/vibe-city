...
import type { Vec3 } from "./bunker-domain";
...
export function LabelSprite({
  ...
  useEffect(() => {
    ...
    if (lines.length <= 1) {
      ...
      ctx.fillStyle = color;
      ctx.font =
        "bold 56px system-ui, -apple-system, Segoe UI, Roboto, sans-serif";
      ...
      const nextWidth = Math.min(
        2048,
        Math.max(256, Math.ceil(maxLineWidth + padX * 2)),
      );
      const nextHeight = Math.min(
        1024,
        Math.max(128, Math.ceil(lines.length * lineHeight + padY * 2)),
      );
      ...
      const desiredHeight = Math.min(
        1.6,
        0.7 + Math.max(0, lines.length - 1) * 0.14,
      );
...
export function Building({
  ...
  function getDoorGeometry(face: Face): {
    position: Vec3;
    ...
    <group position={[center[0], dy / 2, center[2]]}>
      <mesh position={[0, -dy / 2, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial
          color={debug ? debugColors.floor : floorColor}
          transparent
          opacity={opacity}
        />
      </mesh>
      <mesh position={[0, dy / 2, 0]} castShadow receiveShadow>
        <boxGeometry args={[dx, wallThickness, dz]} />
        <meshStandardMaterial
          color={debug ? debugColors.roof : color}
          transparent
          opacity={opacity}
        />
      </mesh>
      <mesh
        castShadow
        receiveShadow
        position={[0, 0, -(dz / 2 - wallThickness / 2)]}
      >
        <Geometry>
          <Base>
          ...
          {doorFace === "north" && isOpen && (
            <Subtraction position={[0, -dy / 2 + doorSize[1] / 2, 0]}>
              <boxGeometry
                args={[doorSize[0], doorSize[1], wallThickness + 0.1]}
              />
            </Subtraction>
          )}
        </Geometry>
        <meshStandardMaterial
          color={debug ? debugColors.north : color}
          transparent
          opacity={opacity}
        />
      </mesh>
      ...
      >
        ...
        <meshStandardMaterial
          color={debug ? debugColors.south : color}
          transparent
          opacity={opacity}
        />
      </mesh>
      <mesh
        castShadow
        receiveShadow
        position={[dx / 2 - wallThickness / 2, 0, 0]}
      >
        <Geometry>
          <Base>
          ...
          {doorFace === "east" && isOpen && (
            <Subtraction position={[0, -dy / 2 + doorSize[1] / 2, 0]}>
              <boxGeometry
                args={[wallThickness + 0.1, doorSize[1], doorSize[0]]}
              />
            </Subtraction>
          )}
        </Geometry>
        <meshStandardMaterial
          color={debug ? debugColors.east : color}
          transparent
          opacity={opacity}
        />
      </mesh>
      ...
      >
        ...
        <meshStandardMaterial
          color={debug ? debugColors.west : color}
          transparent
          opacity={opacity}
        />
      </mesh>
      {showDoor && (
        <mesh
          position={doorGeom.position}
          rotation={doorGeom.rotation}
          castShadow
        >
      ...
      {showDoor && (
        <group position={doorGeom.position} rotation={doorGeom.rotation}>
          <mesh castShadow>
            <boxGeometry
              args={[doorSize[0] + 0.1, doorSize[1] + 0.1, doorThickness * 0.5]}
            />
            <meshStandardMaterial
              color="#2d3748"
              metalness={0.5}
...
export function EnhancedObject({
  position,
  color,
  type,
  visible = true,
  ...
  useFrame((state) => {
    if (ref.current) {
      ref.current.position.y =
        Math.sin(state.clock.elapsedTime * speed) * yOffset;
